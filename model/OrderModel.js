...
const searchOrdersuser = (searchCriteria, id, callback) => {
    // Khởi tạo điều kiện tìm kiếm
    let query = 'SELECT * FROM `order` WHERE UserID = ?'; // Lọc theo UserID
    const values = [id]; // Bắt đầu với id trong danh sách giá trị
    ...
    if (searchCriteria.TimeBuy) {
        query += ' AND TimeBuy >= ?'; // Lọc theo thời gian mua hàng
        values.push(searchCriteria.TimeBuy);
    }
