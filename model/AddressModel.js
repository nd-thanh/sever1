...
const addressModel = {
    ...
    updateAddress: (address, userId, addressType, phone, name, callback) => {
        const query = `
            UPDATE Address 
            SET address = ?, addressType = ?, phone = ?, name = ?
            WHERE UserID = ?`;
        
        db.query(query, [address, addressType, phone, name, userId], (err, results) => {
            if (err) {
                return callback(err, null);
            }
            callback(null, results);
