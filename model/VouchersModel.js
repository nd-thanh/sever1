...
const VoucherModel = {
  ...
  getVoucherByCodeStatus: (code, result) => {
    const query = 'SELECT * FROM voucher WHERE Code = ?';  // Đảm bảo bảng của bạn tên là "voucher" (hoặc đúng tên bảng trong database)
  ...
  getVoucherByCodeStatus: (code, status, callback) => {
    const query = 'SELECT * FROM Voucher WHERE code = ? AND status = ?';
    db.query(query, [code, status], (err, result) => {
      if (err) {
        return callback(err, null);
      }
      // Trả về một kết quả duy nhất (giả sử code là duy nhất)
